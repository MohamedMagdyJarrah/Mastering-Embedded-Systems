...
FIFO_Status_t FIFO_EnqueueItem(FIFO_Buff_t* buffer,ELEMENTS_TYPE* item)
{
	...
	else
	{
		...
		/* Enqueue the item to the new place in the queue */
		strcpy((buffer->data[buffer->head]).FirstName,item->FirstName);
		strcpy((buffer->data[buffer->head]).LastName,item->LastName);
		(buffer->data[buffer->head]).rollNumber = item->rollNumber;
		(buffer->data[buffer->head]).GPA = item->GPA;
		for(i=0 ; i<5 ;i++)
		{
			(buffer->data[buffer->head]).coursesID[i] = item->coursesID[i];
		}
